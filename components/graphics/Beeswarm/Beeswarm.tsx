...
const Beeswarm = ({data, extent, radius, x, removeMiddle}) => {
  ...
  useEffect(() => {
    ...
    const region = data[0].region == "Oceania" || data[0].region == "Asia" ? "Asia & Oceania" : data[0].region;

    const xScale = d3.scaleLog().domain([minimumX, maximumX]).range([margin.left, chartWidth-margin.right]);
    const cScale = inclusiveThresholdScale(costs.map(d => d.cost), costs.map(d => d.color));
    const cLineScale = d3.scaleOrdinal(costs.map(d => d.cost), costs.map(d => d.color));
    const r = d3.scaleSqrt(
      radius,
      [5, Math.sqrt(550 * 200) / 15]
    );
    ...
    const tooltip = d3.select("div.tooltip").empty() ? 
      ...
      .style("z-index", "10")
      .style("visibility", "hidden") : d3.select("div.tooltip");    

    const existingG = svg.select("g");
    const g = existingG.empty() ? svg.append("g").attr("transform", `translate(${[margin.left, 30 + margin.top]})`) : existingG;
    ...
    if (svg.select("g.beeswarm").empty()) {
      ...
      .join("circle")
        ...
        .on("mouseover", function(d,i) {
          ...
          tooltip.html(`<b>${i.data.Country}</b><br>Income: ${Math.round(i.data.Median * 100) / 100}<br>Year: ${i.data.Year}`); 
        })
    ...
    if (svg.select("g.axis").empty()) {
      ...
      .attr('class', 'axis')
      .call(d3.axisBottom(xScale).tickSizeOuter(0).ticks(15)
      ...
      ticks.filter((d, index) => index === 0) 
        .attr("transform", "translate(" +  xScale(zeroCost) + ", 0)");     

      if (removeMiddle) {
        ticks.filter((d, index) => index === 2) 
        .attr("transform", "translate(-100,-9999)"); 
      }
  ...
  return (
    <svg ref={svgRef} viewBox={`0 0 ${chartWidth} ${chartHeight}`} className='chart'></svg>
  )
}
